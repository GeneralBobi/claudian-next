...
function desktopRemoval(h){
 const attrs='data-host="'+h.id+'"';
 return '<details><summary>'+t('Connection access','Bağlantı erişimi')+'</summary><p>'+t('Stop this extension from accessing your notes. Your notes and older connections stay. The extension itself can be uninstalled in Claude.','Bu eklentinin notlarına erişimini durdur. Notların ve eski bağlantıların korunur. Eklentinin kendisini Claude içinden kaldırabilirsin.')+'</p>'+btn('Disconnect','Erişimi kes','remove',false,attrs)+(removing===h.id?'<p role="status">'+t('Disconnect Claudian Next from this folder?','Claudian Next’in bu klasöre erişimi kesilsin mi?')+'</p>'+btn('Cancel','Vazgeç','dismiss-remove')+btn('Disconnect','Erişimi kes','confirm-remove',false,attrs):'')+'</details>';
}
...
document.addEventListener('click',async e=>{const nav=e.target.closest('[data-view]');if(nav&&!busy){view=nav.dataset.view;selectedConnection=null;notice='';await render();return;}const el=e.target.closest('[data-action]');if(!el||busy&&el.dataset.action!=='cancel')return;el.disabled=true;errorBox.hidden=true;try{const a=el.dataset.action;
 ...
 if(a==='add-hosts'){extending=true;plan=null;events=[];complete=false;const d=await api.discover();if(state.localClaudeSetup){state.hosts=state.hosts.filter(h=>h.id==='claude-desktop');d.suggested.hosts=['claude-desktop'];}draft={name:state.profile.name,vault:state.profile.vault,storage:state.profile.storage,mode:'existing',action:'extend',language,hosts:d.suggested.hosts.filter(id=>!state.profile.hosts.some(h=>h.id===id))};await render();}
